...
- **シールアセット**: テーマ別のシール画像（どうぶつ・たべもの・のりもの等）を作成
...
- シール帳の下端切れ・ドラッグ終了判定・ヘッダー重なりを見直し、画面内で使い切れるレイアウトへ調整済み
- ホーム背景とシール画像を、陰影と装飾のあるリッチな見た目へ刷新済み
- シールは SVG から PNG ドット絵へ置き換え、`pixelated` 表示で質感を保つ方針へ更新済み

## ファイル構成

```
static/stickers/            # シール画像アセット
...
- 各シール: PNG ベース。用途に応じてドット絵・イラスト調を使い分ける
- 配置先: `static/stickers/{category}/{name}.png`
